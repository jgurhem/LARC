...
void importBlockBinR(char* filePath, char* sep, double* mat, int bsRow, int bsCol, int gsCol, int startR, int startC, int blockRow, int blockCol){
	FILE * f;
	char matPath[150];
	sprintf(matPath, "%s%d%s%d", filePath, blockRow, sep, blockCol);
	//printf("c%d l%d lr%d lc%d gc%d sr%d sc%d\n", blockCol, blockRow, bsRow, bsCol, gsCol, startR, startC);
	f = fopen(matPath, "r");
	if(f == NULL){
...
double* importMatrixBinR(char* filePath, char* sep, int nbRow, int nbCol, int gsRow, int gsCol, int* size){
	double *m;
	...
	for(int i = 0; i < nbCol; i++){
		for(int j = 0; j < nbRow; j++){
			...
			importBlockBinR(filePath, sep, m, size[2 * (j * nbCol + i) + 1], size[2 * (j * nbCol + i)], gsCol, startR, startC, j, i);
		}
	}
	return m;
}
...
void choice(int bx, int sx, int sy, int nbProc, int nbLocIt, int print, char* Af, char* Bf, char* SFf, char* sep){
	...
	if(Af != 0){
		A = importMatrixBinR(Af, sep, nbProc / bx, bx, size[2 * nbProc + 1], size[2 * nbProc], size);
	}
	if(Bf != 0){
		B = importMatrixBinR(Bf, sep, nbProc / bx, bx, size[2 * nbProc + 1], size[2 * nbProc], size);
	}
    if(SFf != 0){
