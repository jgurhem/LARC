...
void readBlockBinR(char *filePath, char *sep, double *m, int xls, int yls,
  ...
  for (int j = 0; j < zls; j++) {
    for (int i = 0; i < yls; i++) {
      r += fread(m + (zd + j) * ygs * xgs + (yd + i) * xgs + xd, sizeof(double),
                 xls, f);
      if (r < xls) {
...
int *computeParam(int Nx, int Ny, int Nz, int sx, int sy, int sz) {
  ...
  for (int i = 0; i < Nx * Ny * Nz; i++) {
    ...
    // Ny * Nx + y * Nx + x)], size[3 * (z * Ny * Nx + y * Nx + x) + 1], size[3
    // * (z * Ny * Nx + y * Nx + x) + 2]);
  }
...
void choice(int Nx, int Ny, int Nz, int sx, int sy, int sz, int print, char *Af,
            char *Bf, char *SFf, char *sep) {
