...
void genVectorBlockCOO(char *filePath, int nb, int bsize, double *vect,
                       int ind) {
  FILE *f;
  char matPath[150];
...
void genVectorBlockBin(char *filePath, int nb, int bsize, double *vect,
                       int ind) {
  FILE *f;
  char matPath[150];
  sprintf(matPath, "%s%d", filePath, ind);
...
void cleanVector(double *mat) { free(mat); }
